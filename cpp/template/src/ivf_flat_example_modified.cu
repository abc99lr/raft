                                  ...
                                  raft::device_matrix_view<const float, int64_t> queries)
{
  ...
  float* dataset_host = (float*)malloc(dataset.size() * sizeof(float));
  RAFT_CHECK_CUDA(cudaMemcpy(dataset_host, dataset.data(), dataset.size() * sizeof(float)));

  std::cout << "Building IVF-Flat index" << std::endl;
  auto index = ivf_flat::build(
    dev_resources, index_params, dataset_host, dataset.extent(0), dataset.extent(1));
