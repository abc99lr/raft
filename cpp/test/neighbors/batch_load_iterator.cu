...
class AnnBatchLoadIteratorTest : public ::testing::TestWithParam<AnnBatchLoadIteratorInputs<IdxT>> {
 public:
  AnnBatchLoadIteratorTest()
    : stream_(resource::get_cuda_stream(handle_)),
      ps(::testing::TestWithParam<AnnBatchLoadIteratorInputs<IdxT>>::GetParam()),
      dataset(0, stream_)
  {
  }
  ...
  void testBatchedCopy()
  {
    batch_load_iterator<DataT> batches(ps.host_dataset ? host_dataset.data() : dataset.data(),
                                       ...
                                       ps.prefetch);
    IdxT batch_id = 0;
    if (ps.prefetch) { batches.prefetch(); }
 ...
 private:
  ...
  AnnBatchLoadIteratorInputs<IdxT> ps;
  rmm::device_uvector<DataT> dataset;
  std::vector<DataT> host_dataset;
};

const std::vector<AnnBatchLoadIteratorInputs<int64_t>> inputs = {
